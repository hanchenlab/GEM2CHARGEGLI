...
if not "chr22" in infile1 and not "chr22" in infile2 and not "chr22" in snpinfofile:
    ...
    if snpinfofile.endswith(".gz"):
        snpinfofile_handle = gzip.open(snpinfofile, "rb")
    else:
    ...
    if infile1.endswith(".gz"):
        infile1_handle = gzip.open(infile1, "rb")
    else:
    ...
    while line1 and line2:
        ...
        if fields1[columns1["Beta_G"]] == na_string:
            ...
            p_snp_m1_mb = "{:.6}".format(1 - stats.chi2.cdf((float(fields1[columns1["Beta_G"]])/float(fields1[columns1["SE_Beta_G"]]))**2, 1))
            p_snp_m1_robust = "{:.6}".format(1 - stats.chi2.cdf((float(fields1[columns1["Beta_G"]])/float(fields1[columns1["robust_SE_Beta_G"]]))**2, 1))
    ...
    for chr in range(1, 23):
        ...
        if current_snpinfofile.endswith(".gz"):
            snpinfofile_handle = gzip.open(current_snpinfofile, "rb")
        else:
        ...
        if current_infile1.endswith(".gz"):
            infile1_handle = gzip.open(current_infile1, "rb")
        else:
        ...
        if current_infile2.endswith(".gz"):
            infile2_handle = gzip.open(current_infile2, "rb")
        else:
